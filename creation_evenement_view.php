    ...
    <main class="flex-1 overflow-y-auto px-8 py-10">
        <div class="bg-[#18181B] rounded-3xl p-8 border border-[#27272A] max-w-5xl mx-auto shadow-xl">
            ...
            <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'Visiteur'): ?>
                <!-- MESSAGE SI VISITEUR -->
                <div class="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-8 text-center my-10">
                    <div class="w-16 h-16 bg-amber-500/20 rounded-full flex items-center justify-center mx-auto mb-4 text-amber-500">
                        <i data-lucide="lock" class="w-8 h-8"></i>
                    </div>
                    <h4 class="text-xl font-bold text-[#F5F5F7] mb-2">Accès restreint</h4>
                    <p class="text-[#A1A1AA] text-sm">En tant que simple visiteur, vous n'êtes pas autorisé à créer des événements. Vous devez d'abord faire une demande pour devenir membre de l'association depuis votre profil.</p>
                </div>
            <?php else: ?>
                <!-- FORMULAIRE (Si Membre, Admin ou Orga) -->
